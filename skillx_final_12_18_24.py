...
def predict_top_5_job_roles(skills_text):
    ...
    st.info('Embedding your inputs...', icon="🤖")
    skill_embedding = sbert_model.encode(skills_text)
    input_data = tf.convert_to_tensor(skill_embedding, dtype=tf.float32)
    input_data = tf.expand_dims(input_data, axis=0)

    st.info('Generating predictions from the model...', icon="🤖")
    predictions = loaded_model.predict(input_data)
    top_5_indices = np.argsort(predictions[0])[::-1][:3]
        ...
        if not collect_career_goal or not collect_industry or not current_role or not top5_skills:
            ...
            if collect_career_goal == "I want to be promoted":
                ...
                *** Insert divider line here, in between all 3 role objects ***
                ...
                If Gemini replace any role predictions with it's own replacements, introduce a divider line after all 3 job titles and share the following information
                with the user:
